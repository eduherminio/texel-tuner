...
TunableSingle OpenFileRookBonus(46, 8);
TunableSingle SemiOpenFileRookBonus(16, 11);
TunableSingle QueenMobilityBonus(6, 4);
TunableSingle SemiOpenFileKingPenalty(-31, 18);
TunableSingle OpenFileKingPenalty(-100, 17);
TunableSingle KingShieldBonus(23, -10);
TunableSingle BishopPairBonus(31, 82);

TunableArray PassedPawnBonus(
    chess::PieceType::PAWN,
    std::vector<int>{0, -1, -12, -11, 18, 55, 91, 0},
    std::vector<int>{0, 12, 19, 48, 82, 159, 225, 0},
    1,
    1);

TunableArray VirtualKingMobilityBonus(
    chess::PieceType::QUEEN,
    std::vector<int>{0, -385, -75, 41, 46, 33, 31, 26, 21, 18, 20, 14, 12, 4, -5, -15, -30, -37, -33, -31, -26, -26, -13, -33, -19, -5, -17, -44},
    std::vector<int>{0, -193, 60, -6, 2, 22, 13, 7, 12, 10, 13, 18, 15, 18, 19, 18, 17, 13, 6, -2, -10, -23, -32, -44, -55, -74, -95, -96},
    0,
    0);

TunableArray KnightMobilityBonus(
    chess::PieceType::KNIGHT,
    std::vector<int>{0, 23, 36, 45, 53, 60, 70, 81, 100},
    std::vector<int>{0, 50, 81, 94, 105, 118, 116, 110, 93},
    0,
    0);

TunableArray BishopMobilityBonus(
    chess::PieceType::BISHOP,
    std::vector<int>{0, 18, 66, 82, 96, 109, 118, 126, 131, 137, 143, 161, 173, 176, 0},
    std::vector<int>{0, 140, 195, 225, 238, 247, 259, 265, 269, 272, 271, 264, 259, 257, 0},
    0,
    1);

TunableArray RookMobilityBonus(
    chess::PieceType::ROOK,
    std::vector<int>{0, 9, 14, 18, 17, 25, 28, 33, 40, 43, 49, 51, 61, 67, 69},
    std::vector<int>{0, 30, 35, 44, 55, 60, 70, 72, 79, 85, 86, 89, 89, 85, 86},
    0,
    0);
...
int RookAdditonalEvaluation(int squareIndex, int pieceIndex, const u64 opponentPawnAttacks, const chess::Board &board, const chess::Color &color, coefficients_t &coefficients)
{
...
int KnightAdditionalEvaluation(int squareIndex, int pieceIndex, const u64 opponentPawnAttacks, const chess::Board &board, const chess::Color &color, coefficients_t &coefficients)
{
    auto mobilityCount = chess::builtin::popcount(
...
int AdditionalPieceEvaluation(int pieceSquareIndex, int pieceIndex, const u64 opponentPawnAttacks, const chess::Board &board, const chess::Color &color, coefficients_t &coefficients)
{
    switch (pieceIndex)
    {
    case 0:
    ...
    case 1:
    case 7:
        return KnightAdditionalEvaluation(pieceSquareIndex, pieceIndex, opponentPawnAttacks, board, color, coefficients);

    case 3:
    case 9:
        return RookAdditonalEvaluation(pieceSquareIndex, pieceIndex, opponentPawnAttacks, board, color, coefficients);
...
EvalResult Lynx::get_external_eval_result(const chess::Board &board)
{
    ...
    for (int pieceIndex = 0; pieceIndex < 5; ++pieceIndex)
    {
        ...
        while (bitboard != 0)
        {
            ...
            packedScore += AdditionalPieceEvaluation(pieceSquareIndex, pieceIndex, blackPawnAttacks, board, chess::Color::WHITE, coefficients);
    ...
    for (int pieceIndex = 6; pieceIndex < 11; ++pieceIndex)
    {
        ...
        while (bitboard != 0)
        {
            ...
            packedScore -= AdditionalPieceEvaluation(pieceSquareIndex, pieceIndex, whitePawnAttacks, board, chess::Color::BLACK, coefficients);
