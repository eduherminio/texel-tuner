...
const static int numParameters = psqtIndexCount +
                                 ...
                                 // Bucketed arrays
                                 ConnectedRooksBonus.size +                // PSQTBucketCount * 8,
...
public:
    ...
    static parameters_t get_initial_parameters()
    {
        ...
        assert(ConnectedRooksBonus.bucketTunableSize == 8);
...
int RookAdditonalEvaluation(int squareIndex, int pieceIndex, int bucket, int oppositeSideKingSquare, const u64 opponentPawnAttacks, const chess::Board &board, const chess::Color &color, coefficients_t &coefficients)
{
    ...
    if (chess::builtin::popcount(attacks & GetPieceSwappingEndianness(board, chess::PieceType::ROOK, color)) >= 2)
    {
        ...
        packedBonus += ConnectedRooksBonus.packed(bucket, rank);
        IncrementCoefficients(coefficients, ConnectedRooksBonus.index(bucket, rank - ConnectedRooksBonus.start), color);
    }
