...
EvalResult Lynx::get_external_eval_result(const chess::Board &board)
{
    ...
    IncrementCoefficients(coefficients, PieceAttackedByPawnPenalty.index(blackBucket), chess::Color::WHITE, attackedPiecesByBlackPawns);
    IncrementCoefficients(coefficients, PieceAttackedByPawnPenalty.index(whiteBucket), chess::Color::BLACK, attackedPiecesByWhitePawns);

    packedScore += (PieceAttackedByPawnPenalty.packed(blackBucket) * attackedPiecesByBlackPawns) -
                   (PieceAttackedByPawnPenalty.packed(whiteBucket) * attackedPiecesByWhitePawns);
