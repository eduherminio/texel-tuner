...
static int numParameters = psqtIndexCount +
                           ...
                           ProtectedPassedPawnBonus.size +                     // PSQTBucketCount * 6, removing 1 rank values
...
public:
    ...
    static parameters_t get_initial_parameters()
    {
        ...
        ProtectedPassedPawnBonus.add(result);
        ...
        assert(ProtectedPassedPawnBonus.bucketTunableSize == 6);
    ...
    static void print_csharp_parameters(const parameters_t &parameters, const std::array<std::array<tune_t, 12>, PSQTBucketCount> &mobilityPieceValues, bool isFinal = false)
    {
        ...
        name = NAME(ProtectedPassedPawnBonus);
        ProtectedPassedPawnBonus.to_csharp(parameters, ss, name);
    ...
    static void print_cpp_parameters(const parameters_t &parameters, const std::array<std::array<tune_t, 12>, PSQTBucketCount> &mobilityPieceValues, bool isFinal = false)
    {
        ...
        name = NAME(ProtectedPassedPawnBonus);
        ProtectedPassedPawnBonus.to_cpp(parameters, ss, name);
...
int PawnAdditionalEvaluation(int squareIndex, int pieceIndex, int bucket, int sameSideKingSquare, int oppositeSideKingSquare, const u64 sameSidePawnAttacks, const chess::Board &board, const chess::Color &color, coefficients_t &coefficients)
{
    int packedBonus = 0;
...
int AdditionalPieceEvaluation(int pieceSquareIndex, int pieceIndex, int bucket, int sameSideKingSquare, int oppositeSideKingSquare, const u64 sameSidePawnAttacks, const u64 opponentPawnAttacks, const chess::Board &board, const chess::Color &color, coefficients_t &coefficients)
{
    switch (pieceIndex)
    {
    case 0:
    case 6:
        return PawnAdditionalEvaluation(pieceSquareIndex, pieceIndex, bucket, sameSideKingSquare, oppositeSideKingSquare, sameSidePawnAttacks, board, color, coefficients);
...
EvalResult Lynx::get_external_eval_result(const chess::Board &board)
{
    ...
    for (int pieceIndex = 6; pieceIndex < 11; ++pieceIndex)
    {
        ...
        while (bitboard != 0)
        {
            ...
            packedScore -= AdditionalPieceEvaluation(pieceSquareIndex, pieceIndex, blackBucket, blackKing, whiteKing, whitePawnAttacks, blackPawnAttacks, board, chess::Color::BLACK, coefficients);
